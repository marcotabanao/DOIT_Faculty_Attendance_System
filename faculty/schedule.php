...
if (!$active_semester) {
    ...
    $stmt = $pdo->prepare("SELECT s.*, sem.name as semester_name FROM schedules s JOIN semesters sem ON s.semester_id = sem.id WHERE s.faculty_id = ? AND s.semester_id = ? ORDER BY s.day_of_week, s.time_in");
...
$weekly_schedule = [];
foreach ($schedules as $sch) { $weekly_schedule[$sch['day_of_week']][] = $sch; }
?>
<!DOCTYPE html>
<html lang="en">
<head>
    ...
    <div class="row">
        ...
        <div class="col-md-10 p-0">
            ...
            <div class="p-4" style="padding-top: 80px !important;">
                ...
                <p class="text-muted">Semester: <?= htmlspecialchars($active_semester['name'] ?? 'No active semester') ?></p>
                <?= $message ?>
                <?php if (empty($schedules)): ?><div class="alert alert-info">No schedule assigned for this semester.</div>
                <?php else: ?>
                <div class="row">
                    <?php for ($day = 1; $day <= 7; $day++): ?>
                    <div class="col-md-6 col-lg-4 mb-4">
                        <div class="card h-100"><div class="card-header bg-maroon text-white"><h5 class="mb-0"><?= $days[$day] ?></h5></div>
                        <div class="card-body">
                            <?php if (isset($weekly_schedule[$day]) && !empty($weekly_schedule[$day])): ?>
                                ...
                                <div class="mb-3 border-bottom pb-2">
                                    ...
                                    <div><i class="bi bi-clock"></i> <?= formatTime($class['time_in']) ?> - <?= formatTime($class['time_out']) ?></div>
                                ...
                                <?php endforeach; ?>
                            <?php else: ?>
                                <div class="text-center text-muted py-3"><i class="bi bi-calendar-x fs-3"></i><p class="mb-0 mt-2">No classes</p></div>
                            <?php endif; ?>
                        </div></div>
                    </div>
                    <?php endfor; ?>

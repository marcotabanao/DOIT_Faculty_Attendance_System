...
// Get today's attendance status for check-in/out button
$today = date('Y-m-d');
$stmt = $pdo->prepare("SELECT * FROM attendance WHERE faculty_id = ? AND date = ?");
$stmt->execute([$user_id, $today]);
$today_att = $stmt->fetch();
...
<div class="container-fluid">
    <div class="row">
        ...
        <div class="col-md-10 p-4">
            ...
            <!-- Check-in/out panel -->
            <div class="check-card mb-4">
                <div class="row align-items-center">
                    <div class="col-md-6">
                        <h5>Today: <?= date('F d, Y') ?></h5>
                        <?php if ($today_att): ?>
                            <p>Check-in: <?= $today_att['check_in_time'] ? formatTime($today_att['check_in_time']) : 'Not yet' ?></p>
                            <p>Check-out: <?= $today_att['check_out_time'] ? formatTime($today_att['check_out_time']) : 'Not yet' ?></p>
                            <p>Status: <?= getStatusBadge($today_att['status']) ?></p>
                        <?php else: ?>
                    ...
                    <div class="table-responsive">
                        <table class="table table-bordered table-hover">
                            <thead class="table-dark">
                                <tr><th>Date</th><th>Day</th><th>Check In</th><th>Check Out</th><th>Status</th><th>Late (min)</th><th>Remarks</th></tr>
                            </thead>
                            <tbody>
                                <?php if (empty($records)): ?>
                                    <tr><td colspan="7" class="text-center">No attendance records for this month.</td></tr>
                                <?php else: ?>

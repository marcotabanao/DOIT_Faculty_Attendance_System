...
// Fallback constants (in case config/constants.php is incomplete)
if (!defined('LEAVE_ATTACHMENT_PATH')) {
    define('LEAVE_ATTACHMENT_PATH', dirname(__DIR__) . '/assets/uploads/leave_attachments/');
}
        ...
        <div class="col-md-10 p-4">
            ...
            <div class="card mb-4">
                ...
                <div class="card-body text-center">
                    <h2 class="text-primary"><?= number_format($leave_balance, 1) ?></h2>
                    <p class="text-muted">Available leave days</p>
                </div>
            </div>
                            ...
                            <tbody>
                                ...
                                <?php else: ?>
                                    <?php foreach ($leave_requests as $req): ?>
                                        <tr>
                                            <td><?= getLeaveTypeBadge($req['leave_type']) ?></td>
                                            <td><?= formatDate($req['start_date']) ?></td>
                                            <td><?= formatDate($req['end_date']) ?></td>
                                            <td><?= getDateDifference($req['start_date'], $req['end_date']) ?> days</td>
                                            <td><?= nl2br(htmlspecialchars($req['reason'])) ?></td>
                                            <td><?= getLeaveStatusBadge($req['status']) ?></td>
                                            <td>
                                                <?php if ($req['attachment']): ?>
                                                    <a href="../assets/uploads/leave_attachments/<?= htmlspecialchars($req['attachment']) ?>" target="_blank" class="btn btn-sm btn-info">
                                            ...
                                            <td><?= formatDate($req['created_at'], 'Y-m-d H:i') ?></td>
                                        </tr>
                                    <?php endforeach; ?>
                                <?php endif; ?>
                            </tbody>
            ...
            <form method="POST" enctype="multipart/form-data">
                ...
                <div class="modal-body">
                    ...
                    <div class="alert alert-info">
                        <i class="bi bi-info-circle"></i> Your current leave balance: <strong><?= number_format($leave_balance, 1) ?></strong> days.
                    </div>
                </div>
...
<script>
    ...
    startDate.addEventListener('change', function() {
        ...
        if (endDate.value && endDate.value < this.value) endDate.value = '';
    });
    endDate.addEventListener('change', function() {
        if (startDate.value && this.value < startDate.value) this.value = '';
    });
</script>
</body>
</html>
...
function getLeaveTypeBadge($type) {
    ...
    $map = ['sick' => 'info', 'vacation' => 'success', 'emergency' => 'danger', 'maternity' => 'pink', 'paternity' => 'primary', 'other' => 'secondary'];
    return "<span class='badge bg-{$map[$type]}'>" . ucfirst($type) . "</span>";
}
function getLeaveStatusBadge($status) {
    if (empty($status)) return '<span class="badge bg-secondary">Unknown</span>';
    $map = ['pending' => 'warning', 'approved' => 'success', 'rejected' => 'danger'];
    return "<span class='badge bg-{$map[$status]}'>" . ucfirst($status) . "</span>";
}
...
function getSetting($pdo, $key) {
    ...
    $stmt->execute([$key]); return $stmt->fetchColumn();
}
